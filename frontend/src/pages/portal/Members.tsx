...
const Members = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      (membershipsRes.data || []).forEach((m: MemberCompanyMembership) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${deletingMember.first_name} ${deletingMember.last_name} wurde entfernt.`,
      });
    } catch (error) {
  ...
  return (
    <PortalLayout>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center justify-end gap-2">
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => navigate(`/portal/member/${member.id}`)}
      ...
      <MemberManagementDialog
        open={!!managingMember}
        onOpenChange={(open) => {
          if (!open) setManagingMember(null);
        }}
      ...
      <AlertDialog open={!!deletingMember} onOpenChange={() => setDeletingMember(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Sind Sie sicher, dass Sie {deletingMember?.first_name} {deletingMember?.last_name}{" "}
