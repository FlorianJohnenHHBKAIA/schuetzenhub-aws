...
const RolePermissions = () => {
  ...
  const togglePermission = (permissionId: string) => {
    ...
    if (newPending.has(key)) { newPending.delete(key); } else { newPending.add(key); }
  ...
  const discardChanges = () => setPendingChanges(new Set());
  ...
  const RoleList = ({ roleList }: { roleList: Role[] }) => (
    <div className="space-y-2">
      {roleList.length === 0 ? (
        ...
        roleList.map((role) => (
          <button key={role.id} onClick={() => { if (pendingChanges.size > 0) { toast({ title: "Bitte speichern oder verwerfen Sie zuerst die Änderungen" }); return; } setSelectedRole(role); }} className={`w-full p-3 rounded-lg border text-left transition-colors ${selectedRole?.id === role.id ? "bg-primary/10 border-primary" : "bg-card hover:bg-muted/50"}`}>
            <div className="flex items-center justify-between">
              <span className="font-medium">{role.name}</span>
              {role.is_default && <Badge variant="secondary" className="text-xs">Standard</Badge>}
            </div>
            <div className="text-xs text-muted-foreground mt-1">{rolePermissions.filter((rp) => rp.role_id === role.id).length} Rechte</div>
          </button>
        ))
      )}
    </div>
  ...
  const PermissionList = ({ permissionList, category }: { permissionList: Permission[]; category: string }) => (
    <div className="space-y-3">
      ...
      {permissionList.map((permission) => {
        ...
        const isPending = selectedRole ? pendingChanges.has(createKey(selectedRole.id, permission.id)) : false;
        return (
          <div key={permission.id} className={`flex items-start gap-3 p-3 rounded-lg border ${isPending ? "bg-yellow-500/10 border-yellow-500/30" : "bg-card"}`}>
            <Checkbox id={permission.id} checked={isChecked} onCheckedChange={() => togglePermission(permission.id)} disabled={!selectedRole} />
            <div className="flex-1">
              <Label htmlFor={permission.id} className="font-medium cursor-pointer">{permission.key}</Label>
              <p className="text-sm text-muted-foreground">{permission.description}</p>
            </div>
            {isPending && <Badge variant="outline" className="bg-yellow-500/10 text-yellow-600">Geändert</Badge>}
          </div>
        );
      })}
    </div>
              ...
              <div className="bg-card rounded-xl border p-4">
                ...
                <Tabs defaultValue="club">
                  <TabsList className="w-full mb-4">
                    <TabsTrigger value="club" className="flex-1"><Shield className="w-4 h-4 mr-1" />Verein</TabsTrigger>
                    <TabsTrigger value="company" className="flex-1"><Building2 className="w-4 h-4 mr-1" />Kompanie</TabsTrigger>
                  </TabsList>
                  <TabsContent value="club"><RoleList roleList={clubRoles} /></TabsContent>
                  <TabsContent value="company"><RoleList roleList={companyRoles} /></TabsContent>
                </Tabs>
              </div>
            </div>
                  ...
                  <>
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{selectedRole.level === "club" ? "Hauptverein" : "Kompanie"}-Amt</p>
                      </div>
                      {pendingChanges.size > 0 && (
                        <div className="flex gap-2">
                          <Button variant="outline" onClick={discardChanges}>Verwerfen</Button>
                          <Button onClick={saveChanges} disabled={isSaving}>
                            {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4" />Speichern ({pendingChanges.size})</>}
                          </Button>
                        </div>
                      )}
                    </div>
                    <div className="space-y-6">
                      ...
                      <Alert><Info className="w-4 h-4" /><AlertDescription>Kompanie-Rechte wirken nur im jeweiligen Kompanie-Kontext (ab Phase 4/5).</AlertDescription></Alert>
