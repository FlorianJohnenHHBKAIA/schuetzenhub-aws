...
const Events = () => {
  ...
  // Check for share_internal and submit_publication permissions
  const canShareInternalForCompany = (companyId: string) =>
    permissions.some(
      (p) =>
        p.permission_key === "company.events.share_internal" &&
        p.scope_type === "company" &&
        p.scope_id === companyId
    ) || hasPermission("club.admin.full");

  const canSubmitForPublicationForCompany = (companyId: string) =>
    permissions.some(
      (p) =>
        p.permission_key === "company.events.submit_publication" &&
        p.scope_type === "company" &&
        p.scope_id === companyId
    ) || hasPermission("club.admin.full");
  ...
  const fetchData = async () => {
    ...
    try {
      // Fetch club slug for share links
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!ownerId) {
        throw new Error("Keine Kompanie-Zuordnung gefunden");
      }

      // Determine the actual audience based on owner type
      // For club events: can be club_internal or public
      // For company events: can only be company_only or club_internal
      const effectiveAudience: EventAudience = ownerType === "club" 
        ? audience  // Use selected audience for club events
        : (audience === "public" ? "club_internal" : audience); // Company events can't be public directly

      if (editingEvent) {
        // If changing to public, also update publication status
        ...
        // Auto-approve when admin creates public event
        ...
        // Auto-approve public events when created by admin
  ...
  const handleShareInternal = async (event: Event) => {
    try {
      ...
      toast.error(error.message || "Fehler beim Teilen");
    }
  };
        ...
        >
          ...
          {canCreateAnyEvent && (
            <Button onClick={openNewDialog}>
              <Plus className="w-4 h-4 mr-2" /> Termin erstellen
            </Button>
          )}
        </motion.div>
          ...
          <div className="space-y-4">
            {filteredEvents.map((event) => {
              ...
              const showShareButton =
                event.owner_type === "company" &&
                event.audience === "company_only" &&
                canShareInternalForCompany(event.owner_id);

              const showSubmitButton =
                event.owner_type === "company" &&
                event.publication_status === "draft" &&
                canSubmitForPublicationForCompany(event.owner_id);
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          {showShareButton && (
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => handleShareInternal(event)}
                              title="Schützen-intern teilen"
                            >
                          ...
                          {showSubmitButton && (
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => handleSubmitForPublication(event)}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className={`${audience === "public" && ownerType === "club" ? "sm:max-w-2xl" : "sm:max-w-md"} max-h-[90vh] flex flex-col`}>
          ...
          <div className="space-y-4 overflow-y-auto flex-1 pr-2">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    ...
                    // Reset audience when switching owner type
                    if (v === "company") {
                      setAudience("company_only");
                    } else {
                      setAudience("club_internal");
                    }
                  }}
                  ...
                  <SelectContent>
                    ...
                    {canManageCompanyEvents && (
                      <SelectItem value="company">
                        <span className="flex items-center gap-2">
                          <Building2 className="w-4 h-4" /> Meine Kompanie
                        </span>
                      </SelectItem>
                    )}
                  </SelectContent>
            ...
            {/* Audience Selection - only for club events */}
            {ownerType === "club" && canManageClubEvents && (
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="club_internal">
                      <span className="flex items-center gap-2">
                        <Users className="w-4 h-4" /> Schützen-intern
                      </span>
                    </SelectItem>
                    <SelectItem value="public">
                      <span className="flex items-center gap-2">
                        <Globe className="w-4 h-4" /> Öffentlich (Homepage)
                      </span>
                    </SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground mt-1">
                  {audience === "public"
                    ? "Wird automatisch freigegeben und auf der Vereins-Homepage angezeigt"
                    : "Nur sichtbar für angemeldete Vereinsmitglieder"}
                </p>
            ...
            {/* Audience info for company events */}
            {ownerType === "company" && (
              <div>
                <Label>Zielgruppe</Label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="company_only">
                      <span className="flex items-center gap-2">
                        <Building2 className="w-4 h-4" /> Kompanie-intern
                      </span>
                    </SelectItem>
                    <SelectItem value="club_internal">
                      <span className="flex items-center gap-2">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {audience === "company_only"
                    ? "Nur sichtbar für Mitglieder Ihrer Kompanie"
                    : "Sichtbar für alle Vereinsmitglieder"}
                </p>
              </div>
            )}
      ...
      <AlertDialog open={!!deletingEvent} onOpenChange={() => setDeletingEvent(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Sind Sie sicher, dass Sie „{deletingEvent?.title}" löschen möchten?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
