...
const PublicNews = () => {
  ...
  const fetchPublicPosts = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('posts')
        .select(`
          id, club_id, title, content, cover_image_path, category, created_at,
      ...
      if (error) {
        ...
        setPosts(rawPosts);
        if (!slug && rawPosts.length > 0 && rawPosts[0].club?.slug) {
          setClubSlug(rawPosts[0].club.slug);
        }
      }
    } catch (error: unknown) {
  ...
  const getCategoryInfo = (category: string) => CATEGORIES[category] || CATEGORIES.other;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="font-display text-4xl font-bold flex items-center gap-3"><Megaphone className="w-10 h-10" />Aktuelles</h1>
          <p className="text-muted-foreground mt-2">Neuigkeiten und Ankündigungen aus dem Verein</p>
        </motion.div>

        {loading ? (
          <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>
        ) : posts.length === 0 ? (
          <Card><CardContent className="py-12 text-center"><Megaphone className="w-12 h-12 mx-auto mb-4 text-muted-foreground" /><p className="text-muted-foreground">Keine aktuellen Beiträge</p></CardContent></Card>
        ) : (
                  ...
                  <Card className="overflow-hidden hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center gap-2 mb-3">
                        <Badge variant="outline" className="gap-1"><CategoryIcon className="w-3 h-3" />{category.label}</Badge>
