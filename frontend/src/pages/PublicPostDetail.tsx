...
const PublicPostDetail = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: postData, error: postError } = await supabase
          .from("posts")
          .select(`id, title, content, cover_image_path, category, created_at, club_id, creator:members!created_by_member_id(first_name, last_name)`)
          .eq("id", postId)
          .eq("club_id", club.id)
          .eq("audience", "public")
        ...
        if (postError || !postData) { setLoading(false); return; }

        const post = postData as PostData;
        setPost(post);

        if (post.cover_image_path) {
          setCoverImageUrl(getStorageUrl("post-images", post.cover_image_path) || null);
        }
      } catch (error: unknown) {
      ...
      <div className="min-h-screen bg-forest-dark flex flex-col items-center justify-center text-cream">
        ...
        <Link to={`/verein/${slug}`}>
          <Button variant="outline" className="border-gold/30 text-gold hover:bg-gold/10">
            <ArrowLeft className="w-4 h-4 mr-2" />Zurück zur Vereinsseite
          </Button>
        </Link>
      </div>
    );
  ...
  const creatorName = post.creator ? `${post.creator.first_name} ${post.creator.last_name}` : null;
      ...
      <div className="min-h-screen bg-forest-dark">
        <div className="fixed top-0 left-0 right-0 z-50 bg-forest-dark/95 backdrop-blur-sm border-b border-white/10">
          <div className="container mx-auto px-4 h-16 flex items-center justify-between">
            <Link to={`/verein/${slug}`} className="flex items-center gap-2 text-cream/80 hover:text-gold transition-colors">
              <ArrowLeft className="w-5 h-5" />
              <span className="hidden sm:inline">Zurück zu {club.name}</span>
              <span className="sm:hidden">Zurück</span>
            </Link>
            <ShareButtons url={shareUrl} title={post.title} description={ogDescription} variant="compact" className="text-cream/80 hover:text-gold" />
          </div>
        </div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="pt-24 pb-16 px-4">
          <div className="container mx-auto max-w-3xl">
            {coverImageUrl && (
              <div className="rounded-2xl overflow-hidden mb-8">
                <img src={coverImageUrl} alt={post.title} className="w-full h-64 sm:h-80 object-cover" />
              </div>
            )}

            <div className="flex flex-wrap items-center gap-3 mb-4">
              <Badge className="bg-gold/20 text-gold border-gold/30">{categoryLabels[post.category] || post.category}</Badge>
              <span className="text-cream/50 text-sm">{format(new Date(post.created_at), "d. MMMM yyyy", { locale: de })}</span>
            </div>

            <h1 className="font-display text-3xl sm:text-4xl font-bold text-cream mb-6">{post.title}</h1>
            ...
            <div className="prose prose-invert prose-gold max-w-none">
              <p className="text-cream/80 text-lg leading-relaxed whitespace-pre-wrap">{post.content}</p>
            </div>

            <div className="mt-12 pt-8 border-t border-white/10">
              <h3 className="text-cream/60 text-sm mb-4">Beitrag teilen</h3>
              <ShareButtons url={shareUrl} title={post.title} description={ogDescription} />
            </div>

            <div className="mt-12 text-center">
              <Link to={`/verein/${slug}`}>
                <Button size="lg" className="bg-gold hover:bg-gold-light text-forest-dark font-semibold">
                  <ArrowLeft className="w-4 h-4 mr-2" />Alle News von {club.name}
                </Button>
              </Link>
            </div>
          </div>
