...
async function seedUser({ email, password, isSuperAdmin, label }) {
  ...
  if (existing.rows[0]) {
    if (existing.rows[0].is_superadmin !== isSuperAdmin) {
      await pool.query(
        "UPDATE auth_users SET is_superadmin = $1 WHERE id = $2",
        [isSuperAdmin, existing.rows[0].id]
      );
      console.log(`  ->  ${label} (${email}) aktualisiert - is_superadmin: ${isSuperAdmin}`);
      return;
    }
